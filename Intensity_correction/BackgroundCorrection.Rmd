...
Function to plot figures A to F.

```{r}
plotRunsDensities <- function (columnName, xlab, main, legendTitle) {
  xyscale <- density(correctedFluo[which(correctedFluo$batch==1),columnName])
  plot( xyscale
      , ylim=c(0, max(xyscale$y) * 1.1)
      , col=1
      , xlab=xlab
      , main=main
      , type='n')

  linesDensity <- function (runNumber, columnName)
    lines( density(correctedFluo[ which(correctedFluo$batch==runNumber)
                                , columnName])
         , col=runNumber)

  sapply(1:5,linesDensity, columnName=columnName)

  legend("topright", title=legendTitle, c("1772-062-248", "1772-062-249", "1772-064-103", "1772-067-038", "1772-067-039"), lty=1, col=1:5, cex=0.75)
}
```

Figures A to F

```{r figure A, dev=c("svg", "png")}
plotRunsDensities("Ch2_unadjusted"
                 , xlab="Unadjusted log-signal"
                 , main="Figure A: Densities of unadjusted Ch2 signals"
                 , legendTitle="Ch2 signals")
```

```{r figure B, dev=c("svg", "png")}
plotRunsDensities("Ch3_unadjusted"
                 , "Unadjusted log-signal"
                 , "Figure B: Densities of unadjusted Ch3 signals"
                 , "Ch3 signals")
```

```{r figure C, dev=c("svg", "png")}
plotRunsDensities( "ch2_corrected_transformed"
                 , "Adjusted log-signal"
                 , "Figure C: Densities of adjusted Ch2 signals (Flexmix)"
                 , "Ch2 signals")
```

```{r figureD, dev=c("svg", "png")}
plotRunsDensities( "ch3_corrected_transformed"
                 , "Adjusted log-signal"
                 , "Figure D: Densities of adjusted Ch3 signals (Flexmix)"
                 , "Ch3 signals")
```

```{r figureE, dev=c("svg", "png")}
plotRunsDensities( "Ch2_corrected_transformed_anova"
                 , "Adjusted log-signal"
                 , "Figure E: Densities of adjusted Ch2 signals (1-way ANOVA)"
                 , "Ch2 signals")
```

```{r figureF, dev=c("svg", "png")}
plotRunsDensities( "Ch3_corrected_transformed_anova"
                 , "Adjusted log-signal"
                 , "Figure F: Densities of adjusted Ch3 signals (1-way ANOVA)"
                 , "Ch3 signals")
```

Residual histograms (figures H and G)

```{r figureH, dev=c("svg", "png")}
r1 <- c(correctedFluo$Ch2_residuals_anova)
...
```{r figureG, dev=c("svg", "png")}
r2 <- c(correctedFluo$Ch2_flexmix_residuals)
