...
link <- data.frame(link[,-1], intermediate)
names(link) <- c("Library", "DDBJ", "Index")
...
intermediate <- read.table(text=as.character(link$Library), sep='_')
names(intermediate) <- c("Library", "ExperimentAccession")
